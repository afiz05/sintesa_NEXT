...
const DesaFilter = ({ inquiryState }) => {
  ...
  const hasKataFilter = katadesa && katadesa.trim() !== "";
  const hasKondisiFilter = desakondisi && desakondisi.trim() !== "";
  ...
  // Desa selection options
  const DesaSelectionOptions = [
    { value: "XX", label: "Semua Desa" },
    { value: "1114012001", label: "1114012 001 - Keude Teunom" },
    { value: "1114012002", label: "1114012 002 - Alue Ambang" },
  ];
    ...
    <div className="p-3 sm:mx-16 rounded-2xl bg-gradient-to-r from-purple-100 to-fuchsia-100 dark:from-zinc-900 dark:to-zinc-900 shadow-sm">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4 w-full">
        ...
        <h6 className="font-semibold flex items-center gap-2 lg:min-w-[100px] lg:flex-[2]">
          <Home size={20} className="ml-4 text-secondary" />
          Desa
        </h6>
          ...
          <div className="flex flex-col xl:flex xl:flex-row xl:items-end gap-3 xl:gap-4 w-full">
            ...
            <div className="flex flex-col gap-1 w-full xl:flex-1 min-w-0 max-w-full overflow-hidden">
              <div className="flex items-center justify-between">
                ...
                {hasPilihFilter && !isPilihDisabled && (
                  <Button
                    ...
                    onPress={() => setDesavalue && setDesavalue("XX")}
                  >
                    Clear
                  </Button>
                )}
              ...
              <Select
                ...
                size="sm"
                selectedKeys={new Set([desavalue || "XX"])}
                onSelectionChange={(keys) => {
                  const selected = Array.from(keys)[0];
                  if (selected && setDesavalue) {
                    setDesavalue(selected);
                  }
                }}
                isDisabled={isPilihDisabled}
                disallowEmptySelection
              >
                {DesaSelectionOptions.map((opt) => (
            ...
            <div className="flex flex-col gap-1 w-full xl:flex-1">
              <div className="flex items-center justify-between">
                ...
                {hasKondisiFilter && !isKondisiDisabled && (
                  <Button
                    ...
                    color="warning"
                    className="h-6 px-2 text-xs"
                    onPress={() => setDesakondisi && setDesakondisi("")}
                  >
                    Clear
                  </Button>
                )}
              ...
              <Input
                placeholder="misalkan: 1114012001,1114012002, dst"
                className="w-full min-w-0"
                size="sm"
                value={desakondisi || ""}
                isDisabled={isKondisiDisabled}
                onChange={(e) =>
                  setDesakondisi && setDesakondisi(e.target.value)
                }
              />
            </div>
            {/* Kata */}
            <div className="flex flex-col gap-1 w-full xl:flex-1">
              <div className="flex items-center justify-between">
                ...
                {hasKataFilter && !isKataDisabled && (
                  <Button
                    ...
                    color="warning"
                    className="h-6 px-2 text-xs"
                    onPress={() => setKatadesa && setKatadesa("")}
                  >
                    Clear
                  </Button>
                )}
              ...
              <Input
                placeholder="misalkan: keude"
                className="w-full min-w-0"
                size="sm"
                value={katadesa || ""}
                isDisabled={isKataDisabled}
                onChange={(e) => setKatadesa && setKatadesa(e.target.value)}
              />
            </div>
            {/* Jenis Tampilan */}
            <div className="flex flex-col gap-1 w-full xl:flex-1">
              ...
              <Select
                ...
                className="w-full min-w-0"
                size="sm"
                selectedKeys={new Set([desaradio || "1"])}
                onSelectionChange={(keys) => {
                  const selected = Array.from(keys)[0];
                  if (selected && setDesaradio) {
                    setDesaradio(selected);
                  }
                }}
                disallowEmptySelection
              >
