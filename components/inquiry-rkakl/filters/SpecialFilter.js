...
/**
 * Blokir Filter - Handles block type filters for inquiry-rkakl (jenlap = 1)
 */
export class BlokirFilter extends BaseFilter {
  constructor() {
    super("kdblokir", "blokir", {
      schema: "dbref",
      table: "t_blokir",
      alias: "bl",
      nameField: "nmblokir",
      hasYear: true,
      joinCondition: "a.kdblokir=bl.kdblokir",
    });
  }
  ...
  buildFromState(inquiryState) {
    ...
    // For inquiry-rkakl, this filter works with jenlap="1" (Pagu dan Blokir)
    if (jenlap !== "1") {
      return result;
    }

    // Always show both code and name for Blokir filter
    result.columns.push("a.kdblokir", "bl.nmblokir");
    result.groupBy.push("a.kdblokir");

    // Add JOIN clause
    const tableName = this.referenceTable.hasYear
      ? `${this.referenceTable.schema}.${this.referenceTable.table}_${thang}`
      : `${this.referenceTable.schema}.${this.referenceTable.table}`;

    result.joinClause = ` LEFT JOIN ${tableName} ${this.referenceTable.alias} ON ${this.referenceTable.joinCondition}`;

    // Build WHERE conditions if selection is made
    if (pilihValue && pilihValue !== "XX" && pilihValue !== "XXX") {
      result.whereConditions.push(`a.kdblokir = '${pilihValue}'`);
    }
