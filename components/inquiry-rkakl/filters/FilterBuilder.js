...
class FilterBuilder {
  constructor() {
    ...
    this.filters = {
      ...
      komponen: new KomponenFilter(),
      subkomponen: new SubKomponenFilter(),
      item: new ItemFilter(),
  ...
  buildAllFilters(inquiryState) {
    ...
    Object.entries(this.filters).forEach(([key, filter]) => {
      let enabled = false;
      if (key === "blokir") {
        // Enable blokir for jenlap 1 (Pagu dan Blokir) when kdblokir switch is on
        enabled = inquiryState.jenlap === "1" && inquiryState.kdblokir;
