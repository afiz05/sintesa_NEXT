...
const HasilQueryJnsblokir = (props) => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        `${process.env.NEXT_PUBLIC_LOCAL_INQUIRYJNSBLOKIR}${encryptedQuery}&page=${page}&limit=${limit}&user=${username}&thang=${thang}`,
        {
          headers: {
  ...
  data.forEach((row) => {
    ...
    for (
      let cellIndex = startColumnIndex;
  ...
  const renderTableHeaders = () => {
    ...
    return (
      <>
        ...
        {(isThang2024 ? monthHeaders2024 : monthHeadersOther).map(
          (column, index) => (
      ...
      <Modal.Body>
        ...
        {data.length > 0 && (
          <p className="pagination justify-content-between mt-2">
            <span>
              Total: {numeral(rows).format("0,0")}, Hal: {rows ? page + 1 : 0}{" "}
              dari {pages}
            </span>
            <nav>
              <ReactPaginate
